...
public class PokemonHiddenAbilityInventory {
    ...
    public static Inventory get(Pokemon pokemon, Player player) {
        ...
        String pokemonItemName = getFormattedCurrency(SealBuilderLang.getLang("menu.hiddenability.pokemon.name"), currency, price);
        String pokemonItemLore = getFormattedCurrency(SealBuilderLang.getLang("menu.hiddenability.pokemon.lore"), currency, price);
        String cancelItemName = getFormattedCurrency(SealBuilderLang.getLang("menu.hiddenability.cancel.name"), currency, price);
        String cancelItemLore = getFormattedCurrency(SealBuilderLang.getLang("menu.hiddenability.cancel.lore"), currency, price);
        String confirmItemName = getFormattedCurrency(SealBuilderLang.getLang("menu.hiddenability.confirm.name"), currency, price);
        String confirmItemLore = getFormattedCurrency(SealBuilderLang.getLang("menu.hiddenability.confirm.lore"), currency, price);
        ...
        menu.addMenuItem(ClickableItem.create(16, confirmItem).setOnPrimary(click -> {
            ...
            if(MoneyUtils.hasMoney(source, BigDecimal.valueOf(price), currency)) {
                ...
                source.sendMessage(TextUtils.getFormattedText(getFormattedCurrency(SealBuilderLang.getLang("chat.prefix") + SealBuilderLang.getLang("chat.edit.hiddenability"), currency, price)));
    ...
    private static double getHiddenAbilityPrice(Pokemon pokemon, Player player) {
        ...
        return ConfigUtils.applyDiscount(override != null ? override : config.hiddenAbilityPrice, player);
    }
