...
public class PokemonGenderInventory {
    ...
    public static Inventory get(Pokemon pokemon, Player player) {
        ...
        String pokemonItemName = getFormattedCurrency(SealBuilderLang.getLang("menu.gender.pokemon.name"), currency, price);
        String pokemonItemLore = getFormattedCurrency(SealBuilderLang.getLang("menu.gender.pokemon.lore"), currency, price);
        String cancelItemName = getFormattedCurrency(SealBuilderLang.getLang("menu.gender.cancel.name"), currency, price);
        String cancelItemLore = getFormattedCurrency(SealBuilderLang.getLang("menu.gender.cancel.lore"), currency, price);
        String confirmItemName = getFormattedCurrency(SealBuilderLang.getLang("menu.gender.confirm.name"), currency, price);
        String confirmItemLore = getFormattedCurrency(SealBuilderLang.getLang("menu.gender.confirm.lore"), currency, price);
        ...
        menu.addMenuItem(ClickableItem.create(16, confirmItem).setOnPrimary(click -> {
            ...
            if(MoneyUtils.hasMoney(source, BigDecimal.valueOf(price), currency)) {
                ...
                source.sendMessage(TextUtils.getFormattedText(getFormattedCurrency(SealBuilderLang.getLang("chat.prefix") + SealBuilderLang.getLang("chat.edit.gender"), currency, price)));
    ...
    private static double getGenderPrice(Pokemon pokemon, Player player) {
        ...
        return ConfigUtils.applyDiscount(override != null ? override : config.genderPrice, player);
    }
